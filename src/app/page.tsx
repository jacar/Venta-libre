...
const generateFakeOriginalPrice = (price: string, id: number) => {
  ...
  return (current * multiplier).toFixed(2);
};
...
async function getCategoriesData() {
  try {
    const response = await api.get("products/categories", {
      hide_empty: false, // Asegurarnos de traer todas, incluso las vacías
    });
    const allCategories = response.data.filter((c: any) => c.slug !== "sin-categorizar" && c.slug !== "uncategorized");
    ...
    const categoriesWithProducts = await Promise.all(
      allCategories.map(async (cat: any) => {
        try {
          ...
          return { ...cat, products: prodResponse.data };
        } catch (e) {
          return { ...cat, products: [] };
        }
      })
          ...
          <div className="bg-gradient-to-r from-gray-900 to-[#1a1a1a] rounded-2xl overflow-hidden shadow-2xl flex flex-col md:flex-row items-center border border-gray-800">
            <div className="w-full md:w-1/2 relative aspect-[4/3] md:aspect-auto md:h-[400px] bg-white">
              {randomFeaturedProduct.images?.[0] ? (
                <img 
                  src={randomFeaturedProduct.images[0].src} 
                  alt={randomFeaturedProduct.name} 
                  className="w-full h-full object-cover mix-blend-multiply p-8 hover:scale-105 transition-transform duration-500"
                />
              ) : (
              ...
              <div className="flex items-end gap-3 mb-6">
                ...
                {randomFeaturedProduct.regular_price && randomFeaturedProduct.regular_price !== randomFeaturedProduct.price && (
                  <span className="text-xl text-gray-400 line-through mb-1">
                    ${randomFeaturedProduct.regular_price}
                  </span>
                )}
              </div>
