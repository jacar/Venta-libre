...
export default function SportsProductGrid({ products }: { products: any[] }) {
  ...
  // Tomamos los primeros 5 productos para el diseño asimétrico
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 auto-rows-[320px] md:auto-rows-[360px]">
      {displayProducts.map((product: any, idx: number) => {
        ...
        // El primer producto es gigante (ocupa 2 columnas y 2 filas en pantallas medianas y grandes)
          ...
          >
            <Link href={`/product/${product.id}`} className="block w-full h-full relative">
              ...
              <div className="absolute inset-0 w-full h-full overflow-hidden bg-white">
                ...
                {/* Degradado oscuro para que el texto sea legible si el producto es gigante */}
                {isFeatured ? (
                  <div className="absolute inset-0 bg-gradient-to-t from-black/85 via-black/40 to-transparent z-10"></div>
                ) : (
                  <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-transparent to-transparent z-10 opacity-80 group-hover:opacity-100 transition-opacity"></div>
                )}
              </div>
              ...
              {/* Contenido/Información superpuesta sobre la imagen (Estilo Elegante/Luxury) */}
              <div className={`absolute bottom-0 left-0 w-full p-6 sm:p-8 z-25 flex flex-col justify-end text-white`}>
                {isFeatured ? (
                  <>
                    <span className="text-yellow-300 text-xs font-black tracking-widest uppercase mb-1.5 block">ALTO RENDIMIENTO</span>
                    ...
                    <p className="text-white/80 text-sm font-medium mb-5 max-w-md hidden sm:block leading-relaxed">
                      Sube de nivel tu rutina de entrenamiento con el mejor calzado diseñado para máxima comodidad, tracción y durabilidad.
                    </p>
                    <div className="flex items-center justify-between">
                      <div className="flex items-baseline gap-3">
                        <span className="font-black text-xl sm:text-2xl text-yellow-300">{formatCOP(currentPrice)}</span>
                        {oldPrice && oldPrice !== currentPrice && (
                          <span className="text-white/50 text-xs sm:text-sm line-through font-light">{formatCOP(oldPrice)}</span>
                        )}
                      </div>
                  ...
                  <>
                    ...
                    <div className="flex items-baseline justify-between">
                      <div className="flex items-baseline gap-2">
                        <span className="font-black text-base text-white">{formatCOP(currentPrice)}</span>
                        {oldPrice && oldPrice !== currentPrice && (
                          <span className="text-white/50 text-[10px] line-through font-light">{formatCOP(oldPrice)}</span>
                        )}
                      </div>
